...
function AddNewListing() {
    ...
    const nextHandler = async() => {
        ...
        if(data){
            ...
            //console.log("New Data added", data[0].id)
            router.replace('/edit-listing/'+data[0].id)
        } else {
            setLoading(false)
            console.log("Error")
            toast('Something has error')
        }
    }
  return (
    <div className='mt-10 md:mx-56 lg:mx-80'>
        <div className='p-10 flex flex-col gap-5 items-center justify-center'>
            ...
            <div className='p-10 rounded-lg border w-full shadow-md flex flex-col gap-5'>
                <h2 className='text-gray-500'>Enter Address which your want to list</h2>
                <GoogleAddressSearch 
                    selectedAddress={(value)=> setSelectedAddress(value)}
                    setCoordinates={(value)=> setCoordinates(value)}
                />
                <Button onClick={nextHandler} disabled={!selectedAddress || !coordinates || loading}>
